...
for (const key in buttons) {
    if (buttons.hasOwnProperty(key)) {
        ...
        buttons[key].addEventListener('pointerdown', () => {
            buttons[key].classList.add("btn-click");
            const touchCount = parseInt(buttons[key].getAttribute('data-touch-count'));
            buttons[key].setAttribute('data-touch-count', touchCount + 1); // increment click count by 1 for each button

            if (touchCount === 1) {
                buttons[key].classList.remove("btn-click");
            }
            buttons[key].setAttribute('data-touch-count', 0); // reset click count to 0 for each button
